...
/*cd C:\Users\DOSTATIC\Documents\PSyD\wav16b16K
 source load_wav.txt
*/
...
{
    while(((IISFCON) & (0xF << 4)) > (0x6 << 4)); //(mientras IISFCON[7:4] > 6)
    IISFIF = ch0;
    IISFIF = ch1;
}
...
{
	while(((IISFCON) & (0xF)) < 0x2); //(mientras IISFCON[3:0] < 2)
		*ch0 = IISFIF;
	    *ch1 = IISFIF;
}
